...
src/
  main.tsx             # 渲染入口，挂载 BrowserRouter + StrictMode
  App.tsx              # 路由定义（当前仅 / -> Home）
  App.css              # Webapp 专属样式
  index.css            # 全局样式：Tailwind 指令 + shadcn CSS 变量
  pages/               # 页面级组件（当前仅 Home.tsx）
